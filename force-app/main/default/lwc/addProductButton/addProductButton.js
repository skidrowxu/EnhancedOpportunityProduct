...
const oppoLineItemFields = 'Product2.Name, OpportunityLineItem.Product2Id, OpportunityLineItem.Quantity, OpportunityLineItem.UnitPrice, OpportunityLineItem.ServiceDate, OpportunityLineItem.Description'
...
export default class AddProductButton extends NavigationMixin(LightningElement) {
    @api oppoId;
    @api fields;
    cleanedFields;
    isLoading = true;
    ...
    columnMap = [
        [
            (type) => ['ID', 'REFERENCE', 'STRING', 'PICKLIST', 'TEXTAREA'].includes(type),
            (field) => {
                if (field.name === 'Name' && field.objectName === 'Product2' && this.hasProductIdAndProductName) {
                    return {};
                }
                if (field.name === 'Id' && field.objectName === 'Product2' && this.hasProductIdAndProductName) {
    ...
    handleRowSelection(event) {
        switch (event.detail.config.action) {
            ...
            case 'rowSelect': {
                ...
                let selectedRecord = event.detail.selectedRows.find(e => e.PricebookEntryId === event.detail.config.value);
...
//  because it don't need to loop it
    ...
    handleBack(){
        ...
        this.template.querySelector('.errorIcon > lightning-icon[icon-name="utility:error"]').style.visibility = 'hidden';
    ...
    async handleSave(){
        ...
        if (this.errorSentences.length > 0) {
            document.documentElement.style.setProperty('--maxErrorFieldsCount', this.maxErrorFieldsCount);
            document.documentElement.style.setProperty('--errorRowCount', this.errorRowCount);
    ...
    showErrorMsg() {
        ...
        if (!this.clickedFromSave) {
            const errorIcon = this.template.querySelector('.errorIcon > lightning-icon[icon-name="utility:error"]');
    ...
    removeBorderHeaderStyle(isFromChildCmp, hoveredindex, rowIndexFromChildCmp) {
        ...
        if (this.removeIconItem.isClicked) {
            ...
            clickedIndex = this.oppoLineItemFieldMembers.findIndex(el => el.isClicked);
        }
    ...
    handleMouseMoveToBorder(e, borderScope, childCmpValues) {
        ...
        const index = (e === null && columnIndex >= 0 && columnIndex < 6) ? columnIndex : (!isNaN(+e.target.dataset.index) ? +e.target.dataset.index : +e.target.dataset.wrapperIndex);
        const isFromHeader = (e === null && columnIndex >= 0 && columnIndex < 6) ? false : !isNaN(+e.target.dataset.index);

        if (index !== null && (index >= 0 && index < 6)) {
            const rect = e === null ? this.template.querySelector(`[data-index="${index}"]`).getBoundingClientRect() : e.target.getBoundingClientRect();
            ...
            if ((isLeftBorder && index !== 0) || (isRightBorder && index !==5)) {
                ...
                if (index !== 5 && this.oppoLineItemFieldMembers[index].isRightHovering) {
                    this.oppoLineItemFieldMembers[index].isRightHovering = false;
                }
            }
        }
    ...
    handleMouseLeave(e) {
        if (this.currentHoveredIndex !== -1) {
            ...
            if (this.currentHoveredIndex === 5) {
                ...
                this.oppoLineItemFieldMembers[this.currentHoveredIndex].isRightHovering = false;
            }
        }
    }
    ...
    handleMouseDown(eDown, payload) {
        ...
        if (index >= 0 && index <= 5) {
            ...
            const rect = whichPart === 'childCmp' ? fieldDiv.getBoundingClientRect() : eDown.target.getBoundingClientRect();
            if (eDown !== null) {
            ...
            const fieldWrapperDiv = this.template.querySelector(`[data-wrapper-index="${index}"]`);
            ...
            if (this.isInitialRun) {
                document.documentElement.style.setProperty('--restItemSize', orderNumItemDiv.offsetWidth + 'px');
                document.documentElement.style.setProperty('--itemsWidth', (itemsDiv.offsetWidth) + 'px');
                document.documentElement.style.setProperty('--itemWidth', fieldDiv.offsetWidth + 'px');
